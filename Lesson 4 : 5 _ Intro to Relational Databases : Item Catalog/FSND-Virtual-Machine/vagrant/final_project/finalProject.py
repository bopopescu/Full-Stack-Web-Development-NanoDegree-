...
def fbconnect():
    ...
    # See if user exists
    user_id = helper_dao.getUserID(login_session['email'])
    if not user_id:
        user_id = helper_dao.createUser(login_session)
...
# Facebook Disconnect - Revokes a current user's token and reset login_session
...
# Google Disconnect - Revokes a current user's token and reset login_session
...
# Page shows all restaurants
# Loged-in user can edit/delete
...
def editRestaurant(restaurant_id):
    ...
    if request.method == 'POST':
        if request.form['name']:
            ...
            flash('Restaurant "%s" Edited!' % editedRestaurant.name)
            return redirect(url_for('showRestaurants'))
    else:
...
# Authorized users can edit/delete items
...
def showMenu(restaurant_id):
    ...
    creator = helper_dao.getUserInfo(restaurant.user_id)
